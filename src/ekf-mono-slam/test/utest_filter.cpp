...
TEST(ExtendedKalmanFilter, StateAddMapFeature) {
  ...
  const auto inverse_depth_map_feature =
      std::make_shared<InverseDepthMapFeature>(
          feature_state, 6, descriptor_data
      );
  ...
  const std::vector<std::shared_ptr<InverseDepthMapFeature>>
      inverse_depth_features = state.GetInverseDepthFeatures();
  const std::vector<std::shared_ptr<CartesianMapFeature>>
      cartesian_map_features = state.GetCartesianFeatures();
...
TEST(ExtendedKalmanFilter, CovarianceInit) {
  ...
  ASSERT_TRUE(
      covariance_matrix.GetMatrix().diagonal().isApprox(expected_diagonal)
  );
