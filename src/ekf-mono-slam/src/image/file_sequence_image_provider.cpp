...
/**
 * @brief Constructs a FileSequenceImageProvider object for reading images from
 * a sequence.
 *
 * This constructor initializes a new FileSequenceImageProvider object with the
 * specified directory path, indices for image sequence boundaries, and the
 ...
 *
 * This constructor allows setting up an object to efficiently iterate through a
 * sequence of images stored in a directory with a consistent naming convention
 ...
 *
 * This method reads the next image from the specified directory based on a
 * predefined filename format.
 *
 * @return A `cv::Mat` object containing the next image in the sequence, or an
 * empty `cv::Mat` if no more images are available.
 *
...
cv::Mat FileSequenceImageProvider::next() {
  ...
  image_number << std::setw(5) << std::setfill('0')
